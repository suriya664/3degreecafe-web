(function () {
  ...
  const navToggles = selectAll("[data-nav-toggle]");

  // Create backdrop element if it doesn't exist
  let backdrop = select(".site-nav-backdrop");
  if (!backdrop) {
  ...
  const closeNavMenus = () => {
    navToggles.forEach((toggle) => {
      ...
      const nav = header ? select("[data-nav]", header) : null;
      if (!nav) return;
      nav.classList.remove("is-open");
      toggle.classList.remove("is-active");
      toggle.setAttribute("aria-expanded", "false");
    });
    const backdrop = select(".site-nav-backdrop");
    if (backdrop) {
  ...
  doc.addEventListener("click", (event) => {
    ...
    if (!event.target.closest(".site-header") && !event.target.closest(".site-nav-backdrop")) {
      closeNavMenus();
    }
    if (event.target.closest(".site-nav-backdrop")) {
      closeNavMenus();
    }
  });
  ...
  navToggles.forEach((toggle) => {
    ...
    toggle.addEventListener("click", (event) => {
      ...
      if (!isOpen) {
        ...
        const backdrop = select(".site-nav-backdrop");
        if (backdrop && window.innerWidth <= 960) {
  ...
  selectAll(".site-nav a").forEach((link) =>
    link.addEventListener("click", () => {
      if (window.innerWidth <= 960) closeNavMenus();
    })
  );
  ...
  window.addEventListener("resize", () => {
    if (window.innerWidth > 960) {
      ...
      // Ensure backdrop is properly managed on resize
      const nav = select("[data-nav].is-open");
      const backdrop = select(".site-nav-backdrop");
      if (nav && backdrop && window.innerWidth <= 960) {
        backdrop.classList.add("is-active");
        doc.body.classList.add("no-scroll");
      } else if (window.innerWidth > 960 && backdrop) {
        backdrop.classList.remove("is-active");
        doc.body.classList.remove("no-scroll");
      }
    }
